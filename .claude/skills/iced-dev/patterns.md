...
- **tiny-skia の残像は「透過ウィンドウ」と「overlay 機構」の二重原因**（実機で切り分け済み）:
  1. `transparent: true` のウィンドウだと、stack レイヤーで描いたダイアログ/メニューを閉じても
     残像が残る → **不透明テーマでは `transparent: false`** にする
     （`ui.window_opacity < 1.0` のときだけ透過ウィンドウにする）。
  2. 透過を切っても、iced の **overlay 機構で描かれるウィジェット（`tooltip`、`pick_list` の
     ドロップダウン等）は残像化する**。tiny-skia の damage-based redraw が overlay の消去領域を
     登録しないため。→ **overlay 機構を避け、自前の `stack` レイヤーで描く**。
     例: ホバーツールチップは `tooltip` ウィジェットをやめ、`hovered_cell` から文字列を作って
     カーソル位置の container を stack に push する（ダイアログ・コンテキストメニューも同様に
     stack 方式にすれば残像が出ない）。
  3. stack レイヤーにしても、**`container` の `shadow`（blur_radius/offset 付き）は
     ウィジェット bounds の外側に描画される**ため、その要素が動く/消えると影の部分が
     damage 領域に含まれず残像になる（特にカーソル追従する要素で顕著）。
     → **tiny-skia を使うなら shadow は原則使わない**。ツールチップだけでなく、
     コンテキストメニュー・ダイアログ・トーストなど**表示/消去やホバー再描画がある
     オーバーレイ全部**が対象（`container::Style.shadow` を持つスタイルを全て影なしにする）。
     立体感は border（枠）で表現すれば実用上十分。これは iced+tiny-skia 製アプリ全般に
     効く定石（別プロジェクトの同種の残像も同じ対処で直る）。
  4. カーソル追従表示そのものをやめ、**「一定時間ホバー静止で表示・カーソル移動で即消去」**
     にすると、表示中は要素が動かず damage が安定して残像が根絶する（UX も本来こちらが正しい）。
     実装: `last_cursor_move: Option<Instant>` を CursorMoved で更新して表示フラグを落とし、
     `Tick`（100ms）で静止 500ms 経過を検知して表示フラグを立てる。`tick_needed()` に
     「ホバー中かつ未表示」を足して静止待ちの間だけ Tick を購読する。
  切り分けのコツ: マウスをアイテム上でゆっくり動かして軌跡が残るかを SetCursorPos の連続移動 +
  ウィンドウ領域キャプチャで機械的に確認できる。影あり/なし・追従/静止で切り分ける。
